...
class AgenticAiService:
    ...
    def run(self,request:AgenticRequest)->AgenticResponse:
        ...
        return AgenticResponse(run_id=final.run_id,answer=final.answer,final_agent=final.current_agent,tasks=final.tasks,evidence=final.evidence,reasoning_steps=final.reasoning_steps,recommendations=final.recommendations,opportunities=final.opportunities,predictions=final.predictions,revenue_analysis=final.context.get('revenue_analysis'),compliance_review=final.context.get('compliance_review'),coaching_card=final.context.get('coaching_card'),confidence=final.confidence)
